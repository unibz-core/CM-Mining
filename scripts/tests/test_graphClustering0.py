...
index, g = pattern_graphs[0]
print(g.nodes(data=True))
print(g.edges(data=True))


patterns_features = extract_features(pattern_graphs)

print(patterns_features[0])

patterns_dataframe = transform_graph_data(patterns_features)
patterns_similarity_matrix = calculate_similarity(patterns_dataframe)
patterns_cluster_labels = group_similar_items(patterns_similarity_matrix,0.9)
pattern_graphs_clustered_ = merge_lists(patterns_cluster_labels,pattern_graphs)